...
// Missile supply default for each platform
let missileSupply = 10;

// Rate of creating incoming missiles (lower, more missiles)
let rateMissile = 30;

// Maximum incoming missiles
let maxMissiles = 40;

// Big coloud to appear after # of missiles
let bigCloudAppearAfter = 15;
...
function createBigCloud() {
  if (missileCount == bigCloudAppearAfter && bigCloudCount == 0) {
    let y = CANVAS_HEIGHT*0.35;
    let cloud = createSprite(-1000, y, 1000, 456);
    cloud.setSpeed(2, 0);
    cloud.life = 2000;
...
function levelUp() {
  ...
  text('Well done, ' + currentPlayer.playerName + '!', CANVAS_WIDTH/2, CANVAS_HEIGHT*0.3);
  textSize(100);
  text('Score: ' + currentPlayer.currentScore, CANVAS_WIDTH/2, CANVAS_HEIGHT*0.5);
  noLoop();
}
...
function gameInfoBar() {
  ...
  textAlign(CENTER, CENTER);
  text('Score: ' + currentPlayer.currentScore, CANVAS_WIDTH/2, 12);
