...
//
// Created by Toshimitsu Ota
// Illustration by Toshimitsu Ota
...
// Game Title
const GAME_TITLE = 'The Battle of Watercolour';

// Missile supply default for each platform (set 30)
const MISSILE_SUPLY = 30;

// Rate of creating incoming missiles (lower, more missiles) (set 20)
const MISSILE_RATE = 20;

// Maximum incoming missiles (set 40)
const MAX_MISSILES  = 50; // deadult 40

// Big coloud to appear every # missiles
const BIG_CLOUD_RATE = 15;
...
function setup() {
  ...
  video = createVideo(['assets/game-intro.mp4']);
  video.size(1280, 720);
  video.hide();
...
function createBigCloud() {
  // every -- seconds
  if (frameCount % (FRAME_RATE*BIG_CLOUD_RATE) == 0 && missileCount > 0) {
    let directions = ['right', 'left'];
    let direction = random(directions); // Choose direction
    if (direction == 'right') {
      x = - 400;
...
function updateScore() {
  ...
  let topScore = currentPlayer.topScore;
  if (currentScore > topScore) {
    currentPlayer.topScore = currentScore;
    console.log(currentPlayer.topScore);
  }
}
...
// Display number of missles for each platform
...
function segueToGame() {
  ...
  showCountdown = true;
  if (!currentPlayer) {
    currentPlayer = createPlayer(nameInput.value(), 0, 0);
  }
}
...
function displayMainMenu() {
  if (showMainMenu) {
    let menuBoxTop = CANVAS_HEIGHT*0.1;
    noStroke();
    fill(230, 230, 230, 200);
    rect(CANVAS_WIDTH/2, CANVAS_HEIGHT/2, CANVAS_WIDTH*0.6, CANVAS_HEIGHT - menuBoxTop*2);
    noStroke();
    ...
    startButton.style('font-size', '30px');
    textSize(30);
    text('key [1] Leaderboard', CANVAS_WIDTH/2, CANVAS_HEIGHT*0.65);
    text('key [2] Credits', CANVAS_WIDTH/2, CANVAS_HEIGHT*0.65 + 50);
    textSize(25);
    text('Created and Illustrated by Toshimitsu Ota', CANVAS_WIDTH/2, CANVAS_HEIGHT*0.85);
  }
}
...
function displaySplash() {
  if (showSplash) {
    ...
    image(video, (CANVAS_WIDTH-1280)/2, (CANVAS_HEIGHT-720)/2);
    if (video.time() == 0) {
      fill(255);
      textAlign(CENTER, CENTER);
      textSize(50);
      text('Click to Start', CANVAS_WIDTH/2, CANVAS_HEIGHT/2);
    } else if (video.time() >= video.duration()) {
      showMainMenu = true;
      showSplash = false;
    }
  }
